<template>
  <el-dialog v-model="dialogVisible" title="创建Deployment" width="60%" :destroy-on-close="true" @close="handleClose">
    <el-form ref="formRef" :model="formData" :rules="formRules" label-width="120px" style="margin-top: 20px">
      <el-form-item label="名称" prop="name">
        <el-input v-model="formData.name" placeholder="请输入Deployment名称" />
      </el-form-item>

      <el-form-item label="命名空间">
        <el-input :placeholder="`当前命名空间: ${props.namespace || 'default'}`" :value="props.namespace || 'default'"
          disabled />
      ...
      <el-form-item>
        <div class="env-list">
          <div v-for="(env, index) in formData.env" :key="index" class="env-item">
            ...
            <el-input v-model="env.value" placeholder="变量值" style="width: 200px; margin-right: 10px" />
